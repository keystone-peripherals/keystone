...
uintptr_t io_syscall_setsockopt(int socket, int level, int option_name, const void *option_value, socklen_t option_len){
  ...
  size_t totalsize = sizeof(struct edge_syscall) + sizeof(sargs_SYS_getsetsockopt) + option_len;
  ret = dispatch_edgecall_syscall(edge_syscall, totalsize);
...
uintptr_t io_syscall_connect(int sockfd, uintptr_t addr, socklen_t addrlen) {
  ...
  // printf("@@@ addr: ");
  // for (int i = 0; i < args->addrlen; i += 1) printf("0x%02X | ", args->addr[i]);
  // printf("\n");

  size_t totalsize = 
    sizeof(struct edge_syscall)
